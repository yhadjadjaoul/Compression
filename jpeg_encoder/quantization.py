import numpy as np

# Standard JPEG Luminance Quantization Table (Quality 50)
LUMINANCE_TABLE = np.array([
    [16, 11, 10, 16, 24, 40, 51, 61],
    [12, 12, 14, 19, 26, 58, 60, 55],
    [14, 13, 16, 24, 40, 57, 69, 56],
    [14, 17, 22, 29, 51, 87, 80, 62],
    [18, 22, 37, 56, 68, 109, 103, 77],
    [24, 35, 55, 64, 81, 104, 113, 92],
    [49, 64, 78, 87, 103, 121, 120, 101],
    [72, 92, 95, 98, 112, 100, 103, 99]
], dtype=np.float32)

# Standard JPEG Chrominance Quantization Table (Quality 50)
CHROMINANCE_TABLE = np.array([
    [17, 18, 24, 47, 99, 99, 99, 99],
    [18, 21, 26, 66, 99, 99, 99, 99],
    [24, 26, 56, 99, 99, 99, 99, 99],
    [47, 66, 99, 99, 99, 99, 99, 99],
    [99, 99, 99, 99, 99, 99, 99, 99],
    [99, 99, 99, 99, 99, 99, 99, 99],
    [99, 99, 99, 99, 99, 99, 99, 99],
    [99, 99, 99, 99, 99, 99, 99, 99]
], dtype=np.float32)

def get_quantization_table(quality, is_luminance=True):
    """
    Adjust the quantization table based on the quality level.
    """
    if quality <= 0:
        quality = 1
    if quality > 100:
        quality = 100

    if quality < 50:
        scale = 5000 / quality
    else:
        scale = 200 - 2 * quality

    table = LUMINANCE_TABLE if is_luminance else CHROMINANCE_TABLE
    scaled_table = np.floor((table * scale + 50) / 100)
    # Clip values to [1, 255] range as per standard
    scaled_table = np.clip(scaled_table, 1, 255)
    return scaled_table

def quantize(block, table):
    """
    Quantize the DCT block using the given table.
    """
    return np.round(block / table).astype(np.int32)

def dequantize(quantized_block, table):
    """
    Dequantize the block using the given table.
    """
    return (quantized_block * table).astype(np.float32)
